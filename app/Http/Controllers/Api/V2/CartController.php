...
class CartController extends Controller
{
    public function summary($user_id, $owner_id)
    {
        ...
        foreach ($items as $cartItem) {
            ...
            $sum_text += $cartItem->tax * $cartItem->quantity;
            $sum_subtotal += $cartItem->price  * $cartItem->quantity;
        }



        return response()->json([
            'sub_total' => format_price($sum_subtotal),
            'tax' => format_price($sum_text),
...
//    }
    ...
    public function add(Request $request)
    {
        ...
        Cart::updateOrCreate([
         ...
         $cartlist = $this->getList($request->user_id);
         
        return response()->json([
            'result' => true,
            'message' => 'Product added to cart successfully',
            'data'   => $cartlist->original
        ]);
    }
