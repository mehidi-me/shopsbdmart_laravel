...
class AddressController extends Controller
{
    ...
    {
         Address::where('user_id', $request->user_id)->update(['set_default' => 0]);
        $address = new Address;
    ...
    {
     $districts = DB::table('districts')->where('division_id',$id)->select('id','division_id','name')->get();
     return response()->json($districts);
    }
    
    public function getUpazilas($id)
    {
     $upazilas = DB::table('cities')->where('district_id',$id)->select('id','district_id','name')->get();
